...
export default function AdminStudents() {
  ...
  const [filter, setFilter] = useState<"all" | "active" | "warned" | "rejected">("all");

  useEffect(() => {
    const value = new URLSearchParams(window.location.search).get("filter");
    if (value === "active" || value === "warned" || value === "rejected") setFilter(value);
    else setFilter("all");
  }, []);
  ...
  const filteredStudents = students.filter((s) => filter === "all" || s.testStatus === filter);
    ...
    <div className="space-y-6">
        ...
        <div>
          ...
          {filter !== "all" && (
            <p className="text-sm text-[var(--ink-soft)] mt-1">
              Showing students with online-test status: <span className="font-semibold capitalize">{filter}</span>
            </p>
          )}
        </div>
        <button
      ...
      <div className="rounded-2xl border border-[var(--line)] bg-[var(--surface)] divide-y divide-[var(--line)] overflow-hidden">
        {filteredStudents.map((s) => (
          ...
          >
            ...
            <div className="flex items-center gap-3">
              <span className={`text-xs font-semibold px-2.5 py-1 rounded-full capitalize ${statusStyles[filter === "all" ? s.status : s.testStatus]}`}>
                {filter === "all" ? s.status : `test ${s.testStatus}`}
              </span>
        ...
        {filteredStudents.length === 0 && (
          <p className="text-sm text-[var(--ink-faint)] px-6 py-6">
            {filter === "all" ? "No students yet." : `No students with online-test status: ${filter}.`}
          </p>
        )}
      </div>
    </div>
