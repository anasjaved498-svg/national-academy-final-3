...
export default function TakeExam() {
  ...
  useEffect(() => {
    ...
    const onFullscreenChange = () => {
      ...
      setIsFullscreen(fs);
      if (!fs) {
        violate("fullscreen_exit", "You exited fullscreen");
        // Immediately try to re-enter fullscreen rather than waiting for the
        // student to notice and click "Return to Fullscreen" — browsers only
        // allow this to succeed if called very close to a user gesture (e.g.
        // right after they pressed Escape), so it won't always work, but it
        // closes the gap when it does.
      ...
      <div className="max-w-2xl mx-auto py-6">
        ...
        {showSubmitConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
            <div className="max-w-sm w-full rounded-2xl bg-[var(--surface)] border border-[var(--line)] p-6 text-center">
              <p className="font-semibold text-[var(--heading)]">Submit the test now?</p>
              <p className="text-sm text-[var(--ink-soft)] mt-2">
              ...
              <div className="mt-5 flex gap-3">
                <button
                  ...
                  className="flex-1 px-4 py-2.5 rounded-full border border-[var(--line)] text-sm font-medium hover:border-[var(--primary)]"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 px-4 py-2.5 rounded-full bg-[var(--gold)] text-[#1a1200] text-sm font-semibold hover:opacity-90"
                >
                  Submit
                </button>
              </div>
