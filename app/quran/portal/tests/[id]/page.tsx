...
export default function TakeExam() {
  ...
  const { exams, attempts, auth, students, startAttempt, saveAnswer, recordViolation, submitAttempt } = useStore();
  ...
  const violate = useCallback(
    (type: Parameters<typeof recordViolation>[1], message: string) => {
      ...
      // Browsers can fire blur, visibilitychange and fullscreenchange for the
      // same physical exit. Treat that burst as one violation, while allowing
      // a genuinely separate later exit to count again.
      const now = Date.now();
  ...
  useEffect(() => {
    ...
    const onBlur = () => {
      if (!document.hidden) violate("window_blur", "Window lost focus");
    };
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      const fullscreenShortcut = e.key === "F11";
      const blocked =
        e.key === "F12" ||
        (e.ctrlKey && e.shiftKey && ["I", "J", "C"].includes(e.key.toUpperCase())) ||
        (e.ctrlKey && ["U", "P", "S"].includes(e.key.toUpperCase()));
      if (fullscreenShortcut) {
        // F11 is owned by the browser/OS in most browsers and may not reach
        // page JavaScript. When it does, prevent it and keep the test visible.
        e.preventDefault();
        violate("fullscreen_exit", "Fullscreen shortcut is not allowed during the test");
      } else if (blocked) {
        e.preventDefault();
  ...
  if (student?.testStatus === "rejected" && !existingAttempt) {
    return (
      <div className="max-w-lg mx-auto text-center py-16">
        <ShieldAlert size={36} className="mx-auto text-[var(--rose)]" />
        <h1 className="font-display text-2xl mt-4 text-[var(--heading)]">Online tests are unavailable</h1>
        <p className="text-sm text-[var(--ink-soft)] mt-2">
          Your online test access has been rejected by the academy after the required review.
          Please contact the academy if you need clarification.
        </p>
      </div>
    );
  }
    ...
    >
      ...
      {showSubmitConfirm && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/40 p-4">
          <div role="dialog" aria-modal="true" aria-labelledby="submit-test-title" className="w-full max-w-md rounded-2xl border border-[var(--line)] bg-[var(--surface)] p-6 shadow-2xl">
            <h2 id="submit-test-title" className="font-display text-xl text-[var(--heading)]">Submit this test?</h2>
            <p className="text-sm text-[var(--ink-soft)] mt-2">You cannot change your answers after the test is submitted.</p>
            <div className="mt-6 flex justify-end gap-3">
              <button
                type="button"
                onClick={() => setShowSubmitConfirm(false)}
                className="px-5 py-2.5 rounded-full border border-[var(--line)] text-sm font-medium text-[var(--ink-soft)] hover:border-[var(--primary)]"
              >
                Cancel
              </button>
              <button
